...
/// The set of lowercase names we treat as real HTML elements. Capitalised names
/// are components and never reach serialisation. The list is an allowlist on
/// purpose: it keeps the element/component split clean and turns a typo (`dvi`)
/// into a loud error rather than a silent `<dvi>`. Keep it broad enough that you
/// rarely hit the wall; every void element (see `is_void`) must appear here too.
pub fn known_html_tags() -> BTreeSet<&'static str> {
    [
        ...
        "summary",
        "dialog",
        // a basic SVG set, enough for inline icons (namespaced attrs like
        // xmlns:xlink aren't expressible, but viewBox/d/fill/stroke-* are)
        "svg",
        "path",
        "g",
        "circle",
